...
- **Automated Lifecycle Management:** Implements a strict TTL (Time-To-Live) to automatically drop data older than 2 years, optimizing storage costs. Monthly partitioning (toYYYYMM) ensures queries scanning specific date ranges are lightning-fast.
...
| Storage Engine | Standard table | Partitioned by Month with 2-Year TTL | Reduced cloud infrastructure costs and automated data lifecycle management. |
...
| Column | Type | Purpose |
|--------|------|---------|
| **event_time** | `DateTime64(3, 'UTC')` | When the event occurred (client or server); partition/TTL use `toDate(event_time)` |
...
**What is the expected event volume (events/day) and retention period? This affects partitioning and TTL strategy.**
...
- **TTL:** `toDate(event_time) + INTERVAL 2 YEAR` – 2-year retention

**Recommendation:** Define expected volume and retention; adjust TTL and `index_granularity` accordingly.
